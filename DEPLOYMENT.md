...
2. Set `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, and `VAPID_SUBJECT=mailto:hello@madinatydeals.com` in the server environment. Compose passes these variables to the application. Never put the private key in a `VITE_*` variable. The subscription endpoint returns only the public key.
