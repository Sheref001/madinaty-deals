...
- `REGISTRATION_ENABLED=false` pauses new account creation while allowing existing accounts to sign in. The server rejects OTP requests for unknown email addresses while paused. Set it to `true` after OTP delivery is restored to reopen registration.
- Cognito sign-up is integrated as a server-side OpenID Connect authorization-code flow with PKCE. In the Cognito user pool, enable self-service sign-up and verified email, configure the app client for the authorization-code grant and `openid`, `email`, and `profile` scopes, and set its callback URL to exactly `https://madinatydeals.com/api/auth/cognito/callback`; set the sign-out URL to `https://madinatydeals.com/`. The user pool also needs a managed-login domain. The app client ID is public; if that app client has a client secret, put it only in the server `.env` as `COGNITO_CLIENT_SECRET` (leave it blank when no client secret was created). Do not use the homepage with `?lang=ar` as the callback. Set `COGNITO_ISSUER_URL=https://cognito-idp.<region>.amazonaws.com/<user-pool-id>`, `COGNITO_CLIENT_ID`, `COGNITO_CALLBACK_URL=https://madinatydeals.com/api/auth/cognito/callback`, then set both `COGNITO_ENABLED=true` and `REGISTRATION_ENABLED=true` only after the pool, domain, app client, and callback URL are configured. Compose refuses to start Cognito while registration remains paused. With Cognito enabled, the old email-code endpoints are disabled so they cannot bypass Cognito. Cognito identities are linked to local accounts only by verified email; existing roles (including ADMIN) are preserved, and new accounts receive the normal RESIDENT role. The app signs users out of its local session and Cognito managed login. This integration requires a verified email claim; phone-only Cognito identities are not supported by this flow.
