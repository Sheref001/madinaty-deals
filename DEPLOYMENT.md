...
- Cognito-generated confirmation and password-reset codes are sent by **Cognito through SES** after the existing user pool's email configuration is set to `DEVELOPER` with the verified SES domain identity. The website does not need SES SMTP credentials or AWS access keys for this flow. Keep any separate `MAIL_PROVIDER` configuration only for application-generated mail that actually uses the existing mailer.
- Before enabling Cognito delivery through SES, verify the `madinatydeals.com` domain identity in SES in the Region supported by the existing Cognito pool, publish SES's exact DKIM DNS records in Cloudflare, and request production access with mail type **Transactional** and website URL `https://madinatydeals.com`. New SES accounts are sandboxed per Region and can only send to verified recipients until production access is granted. Keep Microsoft 365's existing mail DNS records; merge SPF authorization if SES asks for an SPF record, and do not create a second SPF TXT record at the domain root. See [AWS domain identity setup](https://docs.aws.amazon.com/ses/latest/dg/creating-identities.html) and [production access](https://docs.aws.amazon.com/ses/latest/dg/request-production-access.html).
...
- Current account registration and sign-in use Cognito's managed login. Cognito generates and verifies confirmation and password-reset codes; phone/SMS sign-in is not enabled.
...
Deploy updates from the Ubuntu server with `git pull --ff-only origin main` followed by `docker compose --env-file .env up -d --build --wait`. GitHub Actions deployment is not used. The container entrypoint applies Prisma migrations before starting the app.

### Owner dashboard safety controls

The owner dashboard now uses database-backed totals and action history. Content reviewers can inspect reported, pending, recent and hidden ads; hide an ad immediately; restore it to its prior state; and approve held noncommercial ads. The owner can also remove an ad, suspend/reactivate an account, and pause automatic publication across the site or within a category. Suspension revokes that account's active website sessions and removes its ads and photos from public responses. Pausing publication holds new posts for review; it does not hide existing posts. A pause on **Deals & promotions** holds new service posts with an offer. A moderator needs the explicit `CONTENT_REVIEW` permission for ad actions, `REPORTS` for report decisions and `DASHBOARD` for metrics; publication pauses and account controls remain owner-only.

Use **Hide now** for urgent takedowns. It removes the ad from the public feed, blocks its public image URL and prevents direct content API access. Restoring a previously pending ad returns it to review. Every action records the operator and time; hiding, removing, pausing publication and suspending accounts also require a reason. The review list shows the latest 200 submitted ads; for an older ad, use **Emergency action by ad ID** with the exact ID from its report or link. The browser refreshes its public feed every 15 seconds and when the tab regains focus, so an already-open page can briefly show a stale card; contact actions recheck visibility with the server.

Keep Cloudflare caching disabled for `/api/*`, especially `/api/public-uploads/*`. Public photo responses now say `Cache-Control: private, no-store`; a Cloudflare rule that overrides this would undermine immediate takedown. Purge any old cached `/api/public-uploads/*` objects from before this deployment if such a cache rule was used.
...
Authentication, uploads, submissions, safety moderation and administrator user controls are server-backed. Ordinary submissions publish immediately unless a safety/commercial rule or publication pause holds them for review. Reviewer API routes enforce assigned permissions and record audit actions. Dashboard counts and action history come from the database; advanced demand analytics remain separate work.
