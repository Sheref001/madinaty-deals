...
A bilingual browser notification prompt lets visitors opt in, choose optional offer categories, dismiss for seven days, or turn notifications off. Promotional campaigns now carry an advertiser, offer details, category/zone audience, fee, validity window, review status, payment status, and delivery metrics. Reviewers create and approve campaigns; an admin confirms the manually agreed fee; the sender refuses campaigns that are not approved, paid, and inside their scheduled window. Configure VAPID keys and apply migrations before enabling delivery; Safari on iPhone/iPad requires a Home Screen installation. An operator sending command previews the message/audience by default and sends only with `--send`. See [DEPLOYMENT.md](./DEPLOYMENT.md#browser-notifications). No notifications are sent automatically when an offer is created.
