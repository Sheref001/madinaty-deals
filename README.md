...
A bilingual browser notification prompt lets visitors opt in, dismiss for seven days, or turn notifications off. The push service worker opens notifications on this website only. Configure VAPID keys and apply migrations before enabling delivery; Safari on iPhone/iPad requires a Home Screen installation. An operator sending command previews the message/audience by default and sends only with `--send`. See [DEPLOYMENT.md](./DEPLOYMENT.md#browser-notifications). No notifications are sent automatically when an offer is created.
