...
- `src/data.ts` contains category covers, zones, and formatting helpers, but no sample ads. The client loads published item, service, and online-store submissions from the API; an empty API response shows an empty marketplace.
- Online Finds (`متاجر أونلاين`) is a business category for online shops serving Madinaty by delivery or pickup. Its store-type filter includes Beauty & personal care (`الجمال والعناية الشخصية`), fashion, home/handmade, food, and other online stores. It appears in the home category grid and Businesses discovery view. The posting chooser opens a dedicated online-store form; no physical storefront is required.
...
- API modules are separated by concern: authentication (`auth.js`, `cognito.js`), uploads (`uploads.js`), submissions (`submissions.js`), member account activity (`member-account.js`), accounts/moderators (`admin.js`), moderation/operations (`operations.js`), reports (`reports.js`), and translation (`translation.js`).
- Implemented route groups include health/readiness/config, authentication, uploads and verification, submissions and public submissions, authenticated member account saves/activity/listing management, content visibility/views/comments, reports, translation, and admin users/moderators/verification/content/operations controls. Routes are explicitly matched in `server/app.js`; there is no generated API schema or separate API server.
- Online-store posts use the existing submission route with internal kind `store`, stored in the existing JSON payload (no new table or migration). The server validates a Madinaty service claim, delivery area, store type, WhatsApp number, description, and optional HTTPS social profile on a supported platform. It forces a small-business posting request and `AWAITING_AGREEMENT` fee state, keeping new stores pending commercial review. Public feeds expose approved stores without owner identity or fee state, and map them to business cards; moderation/reporting uses public content type `business`. Owners can manage them under My Listings, while saved items and contact activity use the existing account flows.
...
- Administrators can submit individual Home services, Housekeeping & cleaning, or Local delivery riders services on a provider's behalf after recording a consent attestation and method. These submissions are technically owned by the administrator; the public feed shows the provider name and no verification badge, while the private submission payload and audit log retain the assisted-posting record. The consent attestation is not independent proof of consent. No provider account is created, and a later ownership-transfer workflow is not implemented.
...
- A signed-in member can use one account for buying and selling. `My Account` links to account-synced saved items, contact-open activity, and `My Items`/`My Services`. Guest saved items remain browser-local until sign-in. Contact activity records opening a supported WhatsApp service link and is deliberately not presented as proof of a purchase or booking.
- Owners can edit approved routine listing fields, pause/resume listings, mark items sold, close services, or remove listings. Title, description, and service-area edits return the submission to review; category, provider identity, contact details, photos, promotions, fee state, and verification state are not client-editable. Ownership and optimistic version checks are enforced server-side, and owner changes are audited.
