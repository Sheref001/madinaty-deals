...
type Row = { id: string; kind: string; status: string; title: string; description: string; category: string; owner?: ModeratedContent['owner']; uploadIds: string[]; commercial?: boolean; assistedPosting?: boolean; providerName?: string; createdAt?: string };
...
export default function AdminModeration({ isAdmin, canReadReports }: { isAdmin: boolean; canReadReports: boolean }) {
  ...
  const changeRegistrationAccess = async () => {
    const enabled = !registrationEnabled;
    const reason = enabled ? '' : window.prompt(t('Why are you pausing public sign-in and registration?'))?.trim();
    if (!enabled && (!reason || reason.length < 5)) { setError(t('Enter a reason of at least five characters')); return; }
    setBusy('registration'); setError('');
    try { const result = await setRegistrationAccess(enabled, reason || ''); setRegistrationEnabled(result.enabled); window.dispatchEvent(new Event('madinaty-config-refresh')); }
    catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not change public access'); }
    finally { setBusy(''); }
  };
  ...
  return <section className="admin-panel admin-moderation">
    ...
    {isAdmin && <div className="publication-control"><h3>{t('Post a service for a provider')}</h3><p>{t('Help an individual provider who cannot fill in the form. Get their permission first; their name appears publicly, while your action is recorded privately.')}</p><button className="button button-outline" type="button" aria-expanded={assistedOpen} onClick={() => { setAssistedOpen(value => !value); setAssistedMessage(''); }}>{t(assistedOpen ? 'Close assisted form' : 'Add provider service')}</button>{assistedMessage && <p role="status">{t(assistedMessage)}</p>}{assistedOpen && <ServiceForm assistedPosting onPublish={() => { setAssistedOpen(false); setAssistedMessage('Service submitted. Check its publication status in the list below.'); window.dispatchEvent(new Event('madinaty-operations-refresh')); window.dispatchEvent(new Event('madinaty-feed-refresh')); }} />}</div>}
    {isAdmin && <div className="publication-control registration-access-control"><h3>{t('Public sign-in and registration')}</h3><p>{t(registrationEnabled ? 'Visitors can sign in or create accounts. Turn this off during maintenance; existing listings remain browseable and administrator access stays available.' : 'Public sign-in and registration are paused. Visitors can still browse existing listings; administrator access remains available.')}</p><button className={registrationEnabled ? 'small-action danger-action' : 'small-action approve-action'} disabled={Boolean(busy)} onClick={changeRegistrationAccess}>{t(registrationEnabled ? 'Pause public access' : 'Resume public access')}</button></div>}
    {isAdmin && <div className="publication-control"><h3>{t('Emergency publication pause')}</h3><p>{t('New posts in a paused category are held for review. Existing posts stay visible.')}</p><div className="publication-controls"><select aria-label={t('Publication category')} value={pauseCategory} onChange={event => setPauseCategory(event.target.value)}><option value="*">{t('All categories')}</option>{categories.map(category => <option value={category.label} key={category.label}>{t(category.label)}</option>)}<option value="Moving">{t('Moving')}</option><option value="Private transportation">{t('Private transportation')}</option></select><button className={paused ? 'small-action approve-action' : 'small-action danger-action'} disabled={Boolean(busy)} onClick={() => changePause(!paused)}>{t(paused ? 'Resume publication' : 'Pause publication')}</button></div>{pauses.length > 0 && <p><b>{t('Paused now')}:</b> {pauses.map(item => t(item.category === '*' ? 'All categories' : item.category)).join(' · ')}</p>}</div>}
