...
function AppContent({ onLanguageChange }: { onLanguageChange: (language: Language) => void }) {
  ...
  return (
    <div className="app-shell">
      ...
      {modal === 'post' && <PostModal onClose={() => setModal(null)} onVerify={() => setModal('verify')} onPublish={publishListing} onPublishService={publishService} residentVerified={residentVerified} rentalPostsThisMonth={rentalPostsThisMonth} />}
...
function PostModal({ onClose, onVerify, onPublish, onPublishService, residentVerified, rentalPostsThisMonth }: { onClose: () => void; onVerify: () => void; onPublish: (listing: Listing, published: boolean) => void; onPublishService: (service: Service, published: boolean) => void; residentVerified: boolean; rentalPostsThisMonth: number }) {
  const { t } = useTranslation();
