...
export default function ListingForm({ onPublish, onVerify, residentVerified = false, rentalPostsThisMonth = 0 }: { onPublish: (listing: Listing, published: boolean) => void; onVerify?: () => void; residentVerified?: boolean; rentalPostsThisMonth?: number }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!valid || busy || (isVehicle && !residentVerified)) return;
  ...
  return <form className="modal-form" onSubmit={submit}>
    ...
    {preview ? <section className="listing-preview" aria-label={t('Listing preview')}>
      ...
      {isVehicle && <div className="rental-policy-note"><ShieldCheck size={17} /><span><b>{t('Resident-only vehicle listings')}</b><small>{residentVerified ? t('Your Madinaty residency is verified. You can post a car or motorcycle.') : t('Verify that you live in Madinaty before posting a car or motorcycle.')}</small>{!residentVerified && onVerify && <button className="button button-outline" type="button" onClick={onVerify}>{t('Verify residence')}</button>}</span></div>}
      <div className="form-row"><label>{t('Category')}<select autoFocus value={category} onChange={event => { const nextCategory = event.target.value; setCategory(nextCategory); setCondition(nextCategory === 'Apartment rentals' ? 'Well maintained' : listingConditionOptions(nextCategory)[0]); }}>{categories.filter(item => ['sofa', 'monitor', 'baby', 'car-front', 'building', 'shopping-basket'].includes(item.icon)).map(item => <option value={item.label} key={item.label}>{t(item.label)}</option>)}</select></label>
    ...
    <div className="modal-foot">
      {preview ? <button className="button button-outline" type="button" disabled={busy} onClick={() => setPreview(false)}>{t('Edit details')}</button> : <span className="privacy-note"><ShieldCheck size={15} />{t(isApartmentRental ? 'Apartment details stay private' : 'Verification evidence stays private')}</span>}
      <button className="button button-accent" type="submit" disabled={!valid || busy || (isVehicle && !residentVerified)}>{t(busy ? 'Please wait…' : preview ? 'Publish listing' : 'Preview listing')}<ArrowRight size={16} /></button>
    </div>
