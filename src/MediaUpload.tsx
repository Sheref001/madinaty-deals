...
export default function MediaUpload({ files, onChange }: { files: File[]; onChange: (files: File[]) => void }) {
  ...
  const addFiles = (incoming: File[]) => {
    ...
    if (additions.some(file => !allowedTypes.has(file.type) || file.size > maxBytes)) { setError(t('Use JPG, PNG, WEBP, GIF or AVIF images under 5 MB each.')); return; }
