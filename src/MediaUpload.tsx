...
export default function MediaUpload({ files, onChange }: { files: File[]; onChange: (files: File[]) => void }) {
  ...
  return <section className="media-upload" aria-label={t('Photos')}><div className="media-upload-head"><span><b>{t('Add photos')}</b><small>{t('Up to 6 photos · 5 MB each · 20 MB total')}</small></span><label className="media-upload-button"><ImagePlus size={17} /> {t('Choose photos')}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif" multiple onChange={event => { addFiles(event.target.files); event.currentTarget.value = ''; }} /></label></div>{previews.length > 0 && <div className="media-preview-grid">{previews.map(({ file, url }) => <figure key={`${file.name}-${file.lastModified}`}><img src={url} alt="" /><button type="button" onClick={() => onChange(files.filter(item => item !== file))} aria-label={`${t('Remove')} ${file.name}`}><X size={13} /></button></figure>)}</div>}{files.filter(file => file.type === 'image/heic' || file.type === 'image/heif').map(file => <div className="media-file" key={`${file.name}-${file.lastModified}`}>{file.name}<button type="button" onClick={() => onChange(files.filter(item => item !== file))}><X size={13} /></button></div>)}{error && <small className="form-error">{error}</small>}</section>;
}
