    ...
    {businessOnly ? <p>{t('This category is for businesses. Posting is held until the agreed fee and checks are complete.')}</p>
      : individualOnly ? <p>{t('This category is for individual residents. Business and broker posts are not allowed.')}</p>
        : <div className="posting-audience-options" role="radiogroup" aria-label={t('Who is posting?')}>
          <label className={value === 'individual' ? 'selected' : ''}><input type="radio" name="advertiserType" value="individual" checked={value === 'individual'} onChange={() => onChange('individual')} /><span className="posting-audience-card-content"><b>{t('I am an individual')}</b><small>{t('I am posting for myself.')}</small></span></label>
          <label className={value === 'small_business' ? 'selected' : ''}><input type="radio" name="advertiserType" value="small_business" checked={value === 'small_business'} onChange={() => onChange('small_business')} /><span className="posting-audience-card-content"><b>{t(tutoring ? 'I represent a tutoring centre' : 'I represent a business')}</b><small>{t('I am posting on behalf of a shop, centre or company.')}</small></span></label>
        </div>}
