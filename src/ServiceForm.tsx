...
export default function ServiceForm({ onPublish }: { onPublish: (service: Service, published: boolean) => void }) {
  ...
  return <form className="modal-form" onSubmit={submit}>
    ...
    {!preview && <MediaUpload files={photos} onChange={setPhotos} />}<p className="content-policy-note"><ShieldCheck size={15} />{t('Every post is checked against our safety policy before it is published.')}</p><div className="modal-foot">{preview ? <button className="button button-outline" type="button" disabled={busy} onClick={() => setPreview(false)}>{t('Edit details')}</button> : <span className="privacy-note"><ShieldCheck size={15} />{t('Contact details are used only for enquiries.')}</span>}<button className="button button-accent" type="submit" disabled={!valid || busy}>{t(busy ? 'Please wait…' : preview ? 'Submit for review' : 'Preview service')}<ArrowRight size={16} /></button></div><p className="modal-intro">{t('Your post and photos will be saved privately for review before publishing.')}</p>
  </form>;
}
