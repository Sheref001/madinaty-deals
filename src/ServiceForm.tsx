...
export default function ServiceForm({ onPublish }: { onPublish: (service: Service, published: boolean) => void }) {
  ...
  const specialism = isTutoring ? subjects.slice(0, 2).map(subject => t(subject)).join(' / ')
  ...
  const setupValid = Boolean(category && advertiserType && (!isTutoring || subjects.length > 0));
  const validOffer = !offerEnabled || (offerDiscount.trim().length >= 2 && Boolean(offerValidUntil));
  const valid = setupValid && description.trim().length >= 10 && whatsapp.trim().length >= 8 && validOffer;
  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!setupValid || !valid || busy || stage !== 'preview') return;
    const payload: Service = { id: crypto.randomUUID(), type: 'service', title: title.trim(), subtitle: description.trim(), category, ...(category === 'Tutoring & education' ? { educationLevel, subjects } : {}), ...(category === 'Home services' ? { homeServiceType } : {}), ...(category === 'Housekeeping & cleaning' ? { housekeepingType } : {}), ...(category === 'Health & fitness' ? { fitnessProviderType } : {}), ...(category === 'Pet care' ? { petBusinessType } : {}), advertiserType: advertiserType as AdvertiserType, ...(advertiserType === 'small_business' && !isTutoring ? { businessRequest } : {}), ...(offerEnabled && promotionEligible ? { offer: { kind: offerKind, discount: offerDiscount.trim(), validUntil: offerValidUntil } } : {}), serviceArea: area === 'All zones' ? 'Madinaty-wide' : area, phone: '', whatsapp: whatsapp.trim(), response: availability.trim() || 'Response time to be configured', pricing: price.trim() || undefined, availability: availability.trim() || undefined, rating: 0, reviewCount: 0, zone: area, createdAt: 'Just now', image: 'new-service', accent: 'mint', verified: false };
