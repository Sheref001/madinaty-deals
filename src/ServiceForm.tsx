...
export default function ServiceForm({ onPublish }: { onPublish: (service: Service, published: boolean) => void }) {
  ...
  return <form className="modal-form" onSubmit={submit}>
    ...
    {!preview && <MediaUpload files={photos} onChange={setPhotos} />}<p className="content-policy-note"><ShieldCheck size={15} />{t('Most posts publish immediately. Safety or commercial checks may hold a post for review.')}</p><div className="modal-foot">{preview ? <button className="button button-outline" type="button" disabled={busy} onClick={() => setPreview(false)}>{t('Edit details')}</button> : <span className="privacy-note"><ShieldCheck size={15} />{t('Contact details are used only for enquiries.')}</span>}<button className="button button-accent" type="submit" disabled={!valid || busy}>{t(busy ? 'Please wait…' : preview ? 'Publish service' : 'Preview service')}<ArrowRight size={16} /></button></div><p className="modal-intro">{t('Most posts publish immediately. Safety or commercial checks may hold a post for review.')}</p>
  </form>;
}
