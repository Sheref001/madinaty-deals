...
import { educationLevels, fitnessProviderTypes, homeServiceTypes, housekeepingTypes, tutoringSubjects, type EducationLevel, type FitnessProviderType, type HomeServiceType, type HousekeepingType, type Service, type TutoringSubject } from './types';
...
export default function ServiceForm({ onPublish }: { onPublish: (service: Service) => void }) {
  ...
  const valid = title.trim().length >= 5 && description.trim().length >= 10 && whatsapp.trim().length >= 8 && (!isTutoring || subjects.length > 0) && validOffer;
  async function submit(event: FormEvent) {
    event.preventDefault();
    if (!valid || busy) return;
    if (!preview) { setPreview(true); return; }
    const payload: Service = { id: crypto.randomUUID(), type: 'service', title: title.trim(), subtitle: description.trim(), category, ...(category === 'Tutoring & education' ? { educationLevel, subjects } : {}), ...(category === 'Home services' ? { homeServiceType } : {}), ...(category === 'Housekeeping & cleaning' ? { housekeepingType } : {}), ...(category === 'Health & fitness' ? { fitnessProviderType } : {}), ...((businessOnly || splitCategories.includes(category)) ? { advertiserType: effectiveAdvertiserType, ...(effectiveAdvertiserType === 'small_business' ? { businessRequest } : {}) } : {}), ...(offerEnabled && promotionEligible ? { offer: { discount: offerDiscount.trim(), validUntil: offerValidUntil } } : {}), serviceArea: area, phone: '', whatsapp: whatsapp.trim(), response: availability.trim() || 'Response time to be configured', pricing: price.trim() || undefined, availability: availability.trim() || undefined, rating: 0, reviewCount: 0, zone: area, createdAt: 'Just now', image: 'new-service', accent: 'mint', verified: false };
