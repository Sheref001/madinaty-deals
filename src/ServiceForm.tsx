...
import { educationLevels, fitnessProviderTypes, homeServiceTypes, housekeepingTypes, petBusinessTypes, tutoringSubjects, type EducationLevel, type FitnessProviderType, type HomeServiceType, type HousekeepingType, type PetBusinessType, type Service, type TutoringSubject } from './types';
...
export default function ServiceForm({ onPublish, isAdmin = false }: { onPublish: (service: Service) => void; isAdmin?: boolean }) {
  ...
  const [petBusinessType, setPetBusinessType] = useState<PetBusinessType>('Veterinary clinics');
  ...
  const promotionEligible = isTutoring || category === 'Health & fitness';
  const serviceAreaLabel = area === 'All zones' ? 'All of Madinaty' : area;
  const availableServiceCategories = isAdmin ? serviceCategories : serviceCategories.filter(value => value !== 'Pet care');
  ...
  async function submit(event: FormEvent) {
    ...
    const payload: Service = { id: crypto.randomUUID(), type: 'service', title: title.trim(), subtitle: description.trim(), category, ...(category === 'Tutoring & education' ? { educationLevel, subjects } : {}), ...(category === 'Home services' ? { homeServiceType } : {}), ...(category === 'Housekeeping & cleaning' ? { housekeepingType } : {}), ...(category === 'Health & fitness' ? { fitnessProviderType } : {}), ...(category === 'Pet care' ? { petBusinessType } : {}), ...((businessOnly || splitCategories.includes(category)) ? { advertiserType: effectiveAdvertiserType, ...(effectiveAdvertiserType === 'small_business' ? { businessRequest } : {}) } : {}), ...(offerEnabled && promotionEligible ? { offer: { discount: offerDiscount.trim(), validUntil: offerValidUntil } } : {}), serviceArea: area === 'All zones' ? 'Madinaty-wide' : area, phone: '', whatsapp: whatsapp.trim(), response: availability.trim() || 'Response time to be configured', pricing: price.trim() || undefined, availability: availability.trim() || undefined, rating: 0, reviewCount: 0, zone: area, createdAt: 'Just now', image: 'new-service', accent: 'mint', verified: false };
